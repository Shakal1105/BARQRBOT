...
class Bot():
    def __init__(self):
        self.list = []
        try:
            file = open("dbunique.txt", "r")
            lines = file.readlines()
            for line in lines:
                self.list.append(line[:-1])
        except FileNotFoundError:
            file = open("dbunique.txt", "w")
            file.write("Unique BARCODES:\n")
            file.close()
            ...
            if m.from_user.id in self.admins:
                ...
                if self.checker == -1:
                    if text in self.list:
                        self.bot.send_message(m.chat.id, "Етот товар уже есть в базе")
                    else:
                        self.text_writer = text+"@"
                        self.bot.delete_message(m.chat.id, mid)
                        self.bot.send_message(m.chat.id, f"Штрихкод: {text}\nВведiть назву товару:")
                        self.old_code=text
                        self.new_id = mid+1
                        self.checker=1
                elif self.checker == 1:
                    ...
                    self.bot.delete_message(m.chat.id, mid)
                    self.bot.edit_message_text(chat_id=m.chat.id, message_id=self.new_id, text=f"Штрихкод: {self.old_code}\n{text}\nВведiть кількість товару:")
                    ...
                    try:
                        ...
                        if type(text) is float:
                            ...
                            file.close()
                            file = open("dbunique.txt", "a")
                            file.write(self.old_code + "\n")
                            file.close()
                            self.list.append(self.old_code)
                            if self.help ==1:
            ...
            def script():
                    ...
                    if text == []:
                        ...
                        if text[0].type == "QRCODE":
                            self.bot.send_message(m.chat.id, "обнаружен QR-code:\n\n{}".format(new_text.encode("utf-8")))
                        else:
                            if new_text in self.list:
                                self.bot.send_message(m.chat.id, "Етот товар уже есть в базе")
                            else:
                                time.sleep(1)
                                self.bot.delete_message(m.chat.id, idp)
                                self.text_writer = new_text + "@"
                                self.bot.send_message(m.chat.id, f"Штрихкод: {new_text}\nВведiть назву товару:")
                                self.old_code =new_text
                                self.new_id=idp+1
                                self.checker=1
